...
class Sieve {
    ...
    _onWatchFiles(...diffs) {
        ...
        return this._walkStream(diffs).then(this._write.bind(this)).catch(err=>{
            error(err.message);
            console.error(err);
        });
    }
    ...
    _write(data) {
        ...
        return Promise.all(files.map(({
            ...
            return W(`${OUTPUT}/${NAMESPACE}/${destname}`, content);
        })).then(() => {
            return W(`${OUTPUT}/${NAMESPACE}/resource-map.json`, JSON.stringify(resourceMap.map, null, 4));
        });
    }
}
